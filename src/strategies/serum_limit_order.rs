...
pub async fn place_serum_limit_order(market: &str, price: f64, amount: f64, jito_client: &JitoClient) -> Result<String> {
    let program_id = Pubkey::from_str("9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin")?;
    let tx = Transaction::new_with_payer(&[/* limit order instruction */], None);
    ...
    info!("Placed Serum limit order on {}: {}", market, bundle_id);
    Ok(bundle_id)
}
...
async fn main() -> Result<()> {
    ...
    place_serum_limit_order("SOL/USDC", 100.0, 0.1, &jito).await?;
    Ok(())
}
